...
def pb(input)
  ...
  input.map do |input|
    digit.each do |pattern|
      return input if input.match?(pattern)
    end
  end
end

pattern = /(one)(two)(three)(for)(five)(six)(seven)(eight)(nine)\d/

digit = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine"]
tigit = ["enin", "thgie", "neves", "xis", "evif", "ruof", "eerht", "owt", "eno"]
# p tigid = digit.reverse.map { |d| d.reverse }

p doc = input.first.reverse
p doc.match(/((xis)*(evif)*{1})*\d/)
